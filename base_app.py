...
def main():
    ...
    if selection == "Overview":
        ...
        st.image(
            "https://res.cloudinary.com/limoncloud/image/upload/v1623437693/climate-change-2254711_1920_mvmcum.jpg")
    ...
    if selection == "Team_members":
        ...
        with col1:
            ...
            st.image(
                "https://res.cloudinary.com/limoncloud/image/upload/v1623430480/IMG_20210611_185350_023_fko9ia.jpg",
                caption="EDSA-Student", width=150)
        with col2:
            st.header("Matthew Rip")
            st.image(
                "https://res.cloudinary.com/limoncloud/image/upload/v1624377289/IMG_20210622_175320_507_ss3efp.jpg",
                caption="EDSA-Student", width=150)
        ...
        with col3:
            ...
            st.image(
                "https://res.cloudinary.com/limoncloud/image/upload/v1624197385/IMG_20210620_155017_619_tub8j1.jpg",
                caption="EDSA-Student", width=190)
        with col4:
    ...
    if selection == "Prediction":
        ...
        if st.button("Classify"):
            ...
            predictor1 = joblib.load(open(os.path.join("./lr_model.pkl"), "rb"))
            predictor2 = joblib.load(open(os.path.join("./mlpc_model.pkl"), "rb"))
            predictor3 = joblib.load(open(os.path.join("./nb_model.pkl"), "rb"))
            ...
            # st.success("Logistic Regression - Text Categorized as: {}".format(prediction1))
            st.success("{1}: {0}".format(tweet_classifier(prediction1), 'Logistic Regression'))

            # st.success("MLPClassifier - Text Categorized as: {}".format(prediction2))
            st.success("{1}: {0}".format(tweet_classifier(prediction2), 'MLP Classifier'))

            # st.success("Naive Bayes - Text Categorized as: {}".format(prediction3))
            st.success("{1}: {0}".format(tweet_classifier(prediction3), 'Naive Bayes Classifier'))
